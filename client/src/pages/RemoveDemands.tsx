...
const PATH_NAME = C.REMOVE_DEMANDS;


export default function RemoveDemand() {
  ...
  const add_new = async () => {
    ...
    const fetch_res = await (await fetch(`${tableData.urlPrefix}${C.API_ROOT_PATH}/${PATH_NAME}/?format=json`, {
      ...
      body: JSON.stringify({
        ...
        "dbp:datasetStore": {            // 削除する RealWorldDataset の StoringInfo
          [C.AT_ID]: tableData.urlPrefix + "/api/v0/storing_info/*/?format=json",
          [C.AT_TYPE]: "dbp:RealWorldDataStoringInfo",
        },
        "dbp:timePeriodStart": "-8d",  // 削除する RealWorldDataset のうち対象となるデータの開始時刻はどれだけ前か (ex. 8d → 8日前)
        "dbp:timePeriodEnd": "-7d",    // 削除する RealWorldDataset のうち対象となるデータの終了時刻はどれだけ前か (ex. 7d → 7日前)
      }),
    })).json();
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar
          ...
          helpType={'dbp:RealWorldDataRemoveDemand'}
        />
        <TableContainer>
          <Table
            sx={{ minWidth: 640 }}
              ...
              {visibleRows.map((row: any, idx: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>{getLDLink(row['dbp:datasetStore'], tableData.urlPrefix)}</TableCell>
      ...
      >
        ...
        <DialogContent>
          <Grid container spacing={2} alignItems="center" justifyContent="center" >
            ...
            <HumanFriendlyPropertySingleSelect id={id} pkey={C.DBP_DATASET_STORE} path={C.STORING_INFO} jsonLD={jsonLD} setJsonLD={setJsonLD} urlPrefix={props.urlPrefix} />
