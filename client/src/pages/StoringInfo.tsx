...
import NonStyledLink, { linkStyle } from '../components/NonStyledLink';
import { PageLoader } from '../components/PageLoader';
import CopyToClipboard from '../components/CopyToClipboard';
import JsonLdEditor, { JsonLdEditorProps } from './Editor';
import { decodeIdUrl, encodeIdUrl } from '../etc/utils';
import { formatDateInTimeZone } from '../etc/utils';
import { 
  HumanFriendlyPropertyReadOnly,
  ...
  HumanFriendlyPropertyNumber } from '../components/HumanFriendlyEditors';
...
const PATH_NAME = C.STORING_INFO;


export default function StoringInfo() {
  ...
  const { id } = useParams();
  const { addExplorerDialog } = useFileExplorerContext();
  ...
  const add_new = async () => {
    ...
    const fetch_res = await (await fetch(`${tableData.urlPrefix}${C.API_ROOT_PATH}/${PATH_NAME}/?format=json`, {
      ...
      body: JSON.stringify({
        ...
        "dbp:startTime": createCurrentDateString(),
        "dbp:endTime": createCurrentDateString(),
        "dbp:baseUrl": `file:///mnt/storage/hoge/`,
        "dbp:pattern": `%Y/%Y-%m-%d.csv`,
        "dbp:transmissionSpeed": null,
        "dbp:storageType": null,
        "schema:dateCreated": createCurrentDateString(),
        "dbp:entryPoint": {
          [C.AT_TYPE]: "schema:EntryPoint",
          "schema:urlTemplate": "https://example.com/hoge/%Y/%Y-%m-%d.csv",
        },
      }),
    })).json();
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar
          ...
          helpType={'dbp:RealWorldDataStoringInfo'}
        />
        <TableContainer>
          <Table
            sx={{ minWidth: 640 }}
          ...
          >
            ...
            <TableBody>
              {visibleRows.map((row: any, idx: number) => {
                ...
                // 日付文字列をUTC日付として解析
                const startTime = formatDateInTimeZone(row['dbp:startTime'], C.TIME_ZONE);
                const endTime = formatDateInTimeZone(row['dbp:endTime'], C.TIME_ZONE);
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell><a onClick={() => addExplorerDialog(row['dbp:baseUrl'])} style={linkStyle}>{row['dbp:baseUrl']}</a></TableCell>
                    <TableCell>{row['dbp:pattern']}</TableCell>
      ...
      >
        ...
        <DialogContent>
          <Grid container spacing={2} alignItems="center" justifyContent="center" >
            ...
            <HumanFriendlyPropertyString id={id} pkey={C.DBP_BASE_URL} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertyString id={id} pkey={C.DBP_PATTERN} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertyNumber id={id} pkey={C.DBP_TRANSMISSION_SPEED} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertyString id={id} pkey={C.DBP_STORAGE_TYPE} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyDictProperty id={id} pkey={C.DBP_ENTRY_POINT_KEY} path="" urlPrefix="" jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertyString id={id} pkey={C.DBP_START_TIME} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertyString id={id} pkey={C.DBP_END_TIME} jsonLD={jsonLD} setJsonLD={setJsonLD} />
