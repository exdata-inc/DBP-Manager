...
const PATH_NAME = C.PERIODIC_BREWING_CONFIGS;


export default function PeriodicBrewingConfigs() {
  ...
  const add_new = async () => {
    ...
    const fetch_res = await (await fetch(`${tableData.urlPrefix}${C.API_ROOT_PATH}/${PATH_NAME}/?format=json`, {
      ...
      body: JSON.stringify({
        [C.AT_ID]: "",
        [C.AT_TYPE]: "dbp:RealWorldDataPeriodicBrewingConfig",
        [C.AT_CONTEXT]: {
        ...
        "dbp:brewingConfig": {
          [C.AT_ID]: tableData.urlPrefix + "/api/v0/brewing_demands/*/?format=json",
          [C.AT_TYPE]: "dbp:RealWorldDataRemoveDemand",
        },
        "dbp:cronConfig": "0 0 * * *",
        "dbp:enabled": new Boolean(false),
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar
          ...
          helpType={'dbp:RealWorldDataPeriodicBrewingConfig'}
        />
        <TableContainer>
          <Table
            sx={{ minWidth: 640 }}
              ...
              {visibleRows.map((row: any, idx: number) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell>{getLDLink(row['dbp:brewingConfig'], tableData.urlPrefix)}</TableCell>
                    <TableCell>{row['dbp:cronConfig']}</TableCell>
      ...
      >
        ...
        <DialogContent>
          <Grid container spacing={2} alignItems="center" justifyContent="center" >
            ...
            <HumanFriendlyPropertyBool id={id} pkey={C.DBP_ENABLED} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertySingleSelect id={id} pkey={C.DBP_BREWING_CONFIG} path={C.BREWING_DEMANDS} urlPrefix={props.urlPrefix} jsonLD={jsonLD} setJsonLD={setJsonLD} />
            <HumanFriendlyPropertyString id={id} pkey={C.DBP_CRON_CONFIG} jsonLD={jsonLD} setJsonLD={setJsonLD} />
