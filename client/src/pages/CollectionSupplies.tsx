...
const PATH_NAME = C.COLLECTION_SUPPLIES;


export default function CollectionSupplies() {
  ...
  const add_new = async () => {
    ...
    const fetch_res = await (await fetch(`${tableData.urlPrefix}${C.API_ROOT_PATH}/${PATH_NAME}/?format=json`, {
      ...
      body: JSON.stringify({
        [C.AT_ID]: "",
        [C.AT_TYPE]: "dbp:RealWorldDataCollectionSupply",
        [C.AT_CONTEXT]: {
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar
          ...
          dense={dense}
          setDense={setDense}
          helpType={'dbp:RealWorldDataCollectionSupply'}
        />
        <TableContainer>
          <Table
            sx={{ minWidth: 640 }}
